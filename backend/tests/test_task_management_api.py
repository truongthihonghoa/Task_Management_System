...
def test_task_detail_update_delete_delegate_to_service(monkeypatch):
    db = object()
    user = SimpleNamespace(user_id="USR00000003")
...
def test_get_task_detail_records_recent_view(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "record_recent_view",
        lambda received_db, **kwargs: calls.append((received_db, kwargs)),
    )
    monkeypatch.setattr(task_management_service, "_build_task_detail_response", lambda received_task: received_task)

    response = task_management_service.get_task_detail(db, task.task_id, user)

    assert response is task
    assert calls == [
        (
            db,
            {
...
def test_create_task_rejects_completed_sprint(monkeypatch):
    db = object()
    user = SimpleNamespace(user_id="USR00000003", role="USER")
    ...
    monkeypatch.setattr(task_management_service, "_get_space_or_404", lambda received_db, space_id: space)
    monkeypatch.setattr(task_management_service.task_repository, "get_sprint", lambda received_db, sprint_id: completed_sprint)

    with pytest.raises(HTTPException) as exc_info:
        task_management_service.create_task(db, space.space_id, payload, user)

    assert exc_info.value.status_code == 400
    assert exc_info.value.detail == "Completed sprint is read-only"


def test_update_task_rejects_completed_current_sprint(monkeypatch):
    db = object()
    user = SimpleNamespace(user_id="USR00000003", role="USER")
    space = SimpleNamespace(
        space_id="SPC00000002",
    ...
    task = SimpleNamespace(
        ...
        space_id=space.space_id,
        space=space,
        sprint=SimpleNamespace(status="Completed"),
        deleted_at=None,
    )
    ...
    with pytest.raises(HTTPException) as exc_info:
        task_management_service.update_task(db, task.task_id, TaskUpdate(title="Should not update"), user)

    assert exc_info.value.status_code == 400
    assert exc_info.value.detail == "Completed sprint is read-only"


def test_update_task_rejects_move_to_completed_sprint(monkeypatch):
    db = object()
    user = SimpleNamespace(user_id="USR00000003", role="USER")
    space = SimpleNamespace(
        space_id="SPC00000002",
    ...
    task = SimpleNamespace(
        ...
        space_id=space.space_id,
        space=space,
        sprint=SimpleNamespace(status="Active"),
        deleted_at=None,
    )
    completed_sprint = SimpleNamespace(
        sprint_id="SPR00000009",
        space_id=space.space_id,
        status="Completed",
    )
    ...
    with pytest.raises(HTTPException) as exc_info:
        task_management_service.update_task(db, task.task_id, TaskUpdate(sprint_id=completed_sprint.sprint_id), user)

    assert exc_info.value.status_code == 400
    assert exc_info.value.detail == "Completed sprint is read-only"


def test_delete_task_rejects_completed_sprint(monkeypatch):
    db = object()
    user = SimpleNamespace(user_id="USR00000003", role="USER")
    space = SimpleNamespace(
        space_id="SPC00000002",
    ...
    with pytest.raises(HTTPException) as exc_info:
        task_management_service.delete_task(db, task.task_id, user)
